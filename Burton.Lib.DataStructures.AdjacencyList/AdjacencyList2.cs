...
{
    public class AdjacencyList<EdgeType> where EdgeType : GraphEdge
    {
        private LinkedList<EdgeType>[] InternalAdjacencyList;
        
        // ctor: create empty adjacency list
        public AdjacencyList(int Vertices) 
        {
            InternalAdjacencyList = new LinkedList<EdgeType>[Vertices];

            for (int i = 0; i < InternalAdjacencyList.Length; ++i)
            {
                InternalAdjacencyList[i] = new LinkedList<EdgeType>();
            }
        }
        ...
        {
            InternalAdjacencyList[StartVertex - 1].AddLast(new EdgeType);
        }
